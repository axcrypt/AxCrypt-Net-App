...
e6 1d 44 e1 63 25 1e 20 c7 f6 6e b3 05 11 7c b8
        ...
        {
            TypeMap.Register.Singleton<IRandomGenerator>(() => new InjectedBytesRandomGenerator(seed.FromArgb()));
            ...
            byte[] cipher = keyPair.PublicKey.Transform(message.FromArgb());
            Assert.That(cipher, Is.EquivalentTo(encryption.FromArgb()), testcase);

            byte[] deciphered = keyPair.PrivateKey.Transform(cipher);
            Assert.That(deciphered, Is.EquivalentTo(message.FromArgb()));
        }
    }
}